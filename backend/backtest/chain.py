...
def window_sessions(
    ...
    *,
    ...
    collapsing, which is why the run plots that count (story 77).
...
def burn_in_count(sessions: Sequence[date], measured_start: date | None) -> int:
    ...
    session-count burn-in. ``None`` measures everything (a caller replaying a
    window that is already warm), and a ``measured_start`` after the whole window
    would measure nothing — which is a caller error worth seeing as an empty
    measured result rather than a silently shortened one, so it is not clamped.
    """
